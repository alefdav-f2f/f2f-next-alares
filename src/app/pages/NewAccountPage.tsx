...
export default function NewAccountPage({ hash }: props) {
    ...
    async function validateHash() {
        ...
        const request = await axiosInterceptorInstance.get(`/users/validate-token-to-set-password/${hash}`);
    ...
    async function handlePassword(data: any) {
        ...
        const request: any = await axiosInterceptorInstance.put('/users/set-password', {
                    ...
                    <div className="mb-5 max-w-sm p-8 bg-white border border-gray-200 rounded-lg shadow dark:bg-gray-800 dark:border-gray-700">
                        ...
                        <div>
                            <div className="text-center mb-2">
                                <span>Olá {firstWord(userData.name)}!</span>
                            </div>

                            <div className="text-center mb-2">
                                <span className="text-sm">Crie uma senha para efetuar o primeiro acesso</span>
                            </div>
                        </div>
                        ...
                        <form action="" className="mb-4" onSubmit={handleSubmit(handlePassword)}>
                            ...
                            <div className="mb-4">
                                <label htmlFor="password" className="flex items-center mb-2 text-sm text-gray-900 dark:text-white">
                                    Senha</label>
                                <input type="password" id="user" disabled={isLoading2} {...register('password')} className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" placeholder="" required />
                            </div>
                            ...
                            <div className="flex justify-center mb-1">
                                <button type="submit" disabled={isLoading2} className="text-white bg-main hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5">
                                    {isLoading2 ? <div><Loading /></div> : 'Criar senha'}
                                </button>
                            </div>
                        </form>
